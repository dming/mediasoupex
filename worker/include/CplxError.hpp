#ifndef MS_CPLX_ERROR_HPP
#define MS_CPLX_ERROR_HPP
...
#define SRS_ERRNO_MAP_SYSTEM(XX)                                                                             \
	...
	XX(                                                                                                        \
	  ...
	  1020,                                                                                                    \
	  "RtmpInvalidClient",                                                                                     \
	  "Got invalid RTMP client neither publisher nor player")                                                  \
	...
	XX(                                                                                                        \
	  ERROR_SYSTEM_PID_ALREADY_RUNNING,                                                                        \
	  1035,                                                                                                    \
	  "PidFileProcessExists",                                                                                  \
	  "Exists SRS process specified by pid file")                                                              \
	XX(ERROR_SYSTEM_PID_LOCK, 1036, "PidFileLock", "SRS process exists so that lock pid file failed")          \
	XX(                                                                                                        \
	  ERROR_SYSTEM_PID_TRUNCATE_FILE,                                                                          \
	  1037,                                                                                                    \
	  "PidFileTruncate",                                                                                       \
	  "SRS process exists so that truncate pid file failed")                                                   \
	...
	XX(                                                                                                        \
	  ...
	  1039,                                                                                                    \
	  "PidFileQuery",                                                                                          \
	  "SRS process exists so that query pid file failed")                                                      \
	XX(                                                                                                        \
	  ERROR_SYSTEM_PID_SET_FILE_INFO,                                                                          \
	  1040,                                                                                                    \
	  "PidFileUpdate",                                                                                         \
	  "SRS process exists so that update pid file failed")                                                     \
	...
	XX(                                                                                                        \
	  ERROR_SYSTEM_HOURGLASS_RESOLUTION,                                                                       \
	  1065,                                                                                                    \
	  "TimerResolution",                                                                                       \
	  "Resolution for timer or hourglass is invalid")                                                          \
...
#define SRS_ERRNO_MAP_RTMP(XX)                                                                          \
	...
	XX(ERROR_RTMP_SOUP_ERROR, 2056, "RtmpSoup", "RtmpSoup Error")                                         \
...
#define SRS_ERRNO_MAP_HTTP(XX)                                                                             \
	...
	XX(                                                                                                      \
	  ERROR_STREAM_CASTER_HEVC_FORMAT,                                                                       \
	  4057,                                                                                                  \
	  "CasterTsHevcFormat",                                                                                  \
	  "Invalid ts HEVC Format for stream caster")
...
enum SrsErrorCode
{
	...
	ERROR_SUCCESS = 0,
#endif
	SRS_ERRNO_MAP_SYSTEM(SRS_ERRNO_GEN) SRS_ERRNO_MAP_RTMP(SRS_ERRNO_GEN)
	  SRS_ERRNO_MAP_APP(SRS_ERRNO_GEN) SRS_ERRNO_MAP_HTTP(SRS_ERRNO_GEN)
	    SRS_ERRNO_MAP_RTC(SRS_ERRNO_GEN) SRS_ERRNO_MAP_SRT(SRS_ERRNO_GEN)
...
public:
	static SrsCplxError* create(
	  const char* func, const char* file, int line, int code, const char* fmt, ...);
	static SrsCplxError* wrap(
	  const char* func, const char* file, int line, SrsCplxError* err, const char* fmt, ...);
