#ifndef MS_RTMP_CODEC_HPP
#define MS_RTMP_CODEC_HPP
...
namespace RTMP
{
	...
	enum SrsVideoCodecId
	{
		...
		// See page 79 at @doc
		// https://github.com/CDN-Union/H265/blob/master/Document/video_file_format_spec_v10_1_ksyun_20170615.doc
	...
	enum SrsAudioAacFrameTrait
	{
		...
		// For MP3 we assume the first packet is sequence header, while it actually is not the same
		// thing, because we do this to simplify the workflow, to make sure we can detect the audio
		// codec from the sequence headers.
...
#define SrsHevcNaluTypeParse(code) (SrsHevcNaluType)((code & 0x7E) >> 1)
	...
	private:
		...
		virtual srs_error_t video_avc_demux(Utils::RtmpBuffer* stream, int64_t timestamp);
#ifdef SRS_H265
	private:
		virtual srs_error_t hevc_demux_hvcc(Utils::RtmpBuffer* stream);
	...
	private:
		...
		virtual srs_error_t hevc_demux_rbsp_ptl(
	...
	public:
		virtual srs_error_t hevc_demux_vps(Utils::RtmpBuffer* stream);
		virtual srs_error_t hevc_demux_sps(Utils::RtmpBuffer* stream);
		virtual srs_error_t hevc_demux_pps(Utils::RtmpBuffer* stream);
#endif
	private:
		// Parse the H.264 SPS/PPS.
		virtual srs_error_t avc_demux_sps_pps(Utils::RtmpBuffer* stream);
	...
	private:
		...
		virtual srs_error_t video_nalu_demux(Utils::RtmpBuffer* stream);
		// Demux the avc NALU in "AnnexB" from ISO_IEC_14496-10-AVC-2003.pdf, page 211.
		virtual srs_error_t avc_demux_annexb_format(Utils::RtmpBuffer* stream);
		virtual srs_error_t do_avc_demux_annexb_format(Utils::RtmpBuffer* stream);
		// Demux the avc NALU in "ISO Base Media File Format" from ISO_IEC_14496-15-AVC-format-2012.pdf, page 20
		virtual srs_error_t avc_demux_ibmf_format(Utils::RtmpBuffer* stream);
		virtual srs_error_t do_avc_demux_ibmf_format(Utils::RtmpBuffer* stream);

	private:
		// Demux the audio packet in AAC codec.
		//          Demux the asc from sequence header.
		//          Demux the sampels from RAW data.
		virtual srs_error_t audio_aac_demux(Utils::RtmpBuffer* stream, int64_t timestamp);
		virtual srs_error_t audio_mp3_demux(Utils::RtmpBuffer* stream, int64_t timestamp, bool fresh);
