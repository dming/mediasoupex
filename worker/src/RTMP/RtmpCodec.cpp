#define MS_CLASS "RTMP::RtmpCodec"
...
namespace RTMP
{
	...
	bool RtmpFlvVideo::keyframe(char* data, int size)
	{
		// 2bytes required.
		if (size < 1)
		{
	...
	bool RtmpFlvVideo::sh(char* data, int size)
	{
	...
	bool RtmpFlvVideo::h264(char* data, int size)
	{
		// 1bytes required.
		if (size < 1)
		{
...
#ifdef SRS_H265
	bool RtmpFlvVideo::hevc(char* data, int size)
	{
		// 1bytes required.
		if (size < 1)
		{
	...
	bool RtmpFlvVideo::acceptable(char* data, int size)
	{
		// 1bytes required.
		if (size < 1)
		{
	...
	bool RtmpFlvAudio::sh(char* data, int size)
	{
	...
	bool RtmpFlvAudio::aac(char* data, int size)
	{
		// 1bytes required.
		if (size < 1)
		{
	...
	int srs_aac_srates[] = { 96000, 88200, 64000, 48000, 44100, 32000, 24000, 22050,
	...
	srs_error_t SrsSample::parse_bframe()
	{
		...
		Utils::RtmpBuffer* stream = new Utils::RtmpBuffer(bytes, size);
		RtmpAutoFree(Utils::RtmpBuffer, stream);

		// Skip nalu header.
		stream->skip(1);

		Utils::RtmpBitBuffer bitstream(stream);
		...
		{
			bframe = true;
			MS_DEBUG_DEV_STD("nal_type=%d, slice type=%d", nal_type, slice_type);
		}
	...
	srs_error_t SrsVideoFrame::add_sample(char* bytes, int size)
	{
		...
		if (c && c->id == SrsVideoCodecIdHEVC)
		{
			...
			has_idr                   = (SrsHevcNaluType_CODED_SLICE_BLA <= nalu_type) &&
			          (nalu_type <= SrsHevcNaluType_RESERVED_23);
			return err;
#else
	...
	srs_error_t SrsFormat::on_audio(int64_t timestamp, char* data, int size)
	{
		...
		if (!data || size <= 0)
		{
			MS_DEBUG_DEV_STD("no audio present, ignore it.");
			return err;
		}

		Utils::RtmpBuffer* buffer = new Utils::RtmpBuffer(data, size);
		RtmpAutoFree(Utils::RtmpBuffer, buffer);

		// We already checked the size is positive and data is not nullptr.
	...
	srs_error_t SrsFormat::on_video(int64_t timestamp, char* data, int size)
	{
		...
		if (!data || size <= 0)
		{
			MS_DEBUG_DEV_STD("no video present, ignore it.");
			return err;
		}
	...
	{
		return acodec && acodec->id == SrsAudioCodecIdAAC && audio &&
		       audio->aac_packet_type == SrsAudioAacFrameTraitSequenceHeader;
	}

	bool SrsFormat::is_mp3_sequence_header()
	{
		return acodec && acodec->id == SrsAudioCodecIdMP3 && audio &&
		       audio->aac_packet_type == SrsAudioMp3FrameTraitSequenceHeader;
	}
	...
	bool SrsFormat::is_avc_sequence_header()
	{
		...
		return vcodec && (h264 || h265 || av1) && video &&
		       video->avc_packet_type == SrsVideoAvcFrameTraitSequenceHeader;
	}

	srs_error_t SrsFormat::video_avc_demux(Utils::RtmpBuffer* stream, int64_t timestamp)
	{
		srs_error_t err = srs_success;
		...
		{
			MS_WARN_DEV("avc igone the info frame");
			return err;
		}
		...
		if (!codec_ok)
		{
			return srs_error_new(
			  ERROR_HLS_DECODE_ERROR, "only support video H.264/H.265, actual=%d", codec_id);
		}
		vcodec->id = codec_id;
	...
	// For media server, we don't care the codec, so we just try to parse sps-pps, and we could ignore
	// any error if fail. LCOV_EXCL_START

#ifdef SRS_H265
	// Parse the hevc vps/sps/pps
	srs_error_t SrsFormat::hevc_demux_hvcc(Utils::RtmpBuffer* stream)
	{
		srs_error_t err = srs_success;
		...
		{
			return srs_error_new(
			  ERROR_HEVC_DECODE_ERROR, "requires %d only %d bytes", HEVC_MIN_SIZE, stream->left());
		}
		...
		{
			return srs_error_new(
			  ERROR_HEVC_DECODE_ERROR, "invalid version=%d", dec_conf_rec_p->configuration_version);
		}
		...
		MS_DEBUG_DEV_STD(
		  "hevc version:%d, general_profile_space:%d, general_tier_flag:%d, general_profile_idc:%d",
		  dec_conf_rec_p->configuration_version,
		...
		MS_DEBUG_DEV_STD(
		  "general_constraint_indicator_flags:0x%x, general_level_idc:%d, min_spatial_segmentation_idc:%d, parallelism_type:%d, chroma_format:%d, bit_depth_luma_minus8:%d, bit_depth_chroma_minus8:%d",
		  ...
		  dec_conf_rec_p->parallelism_type,
		  dec_conf_rec_p->chroma_format,
		  dec_conf_rec_p->bit_depth_luma_minus8,
		  dec_conf_rec_p->bit_depth_chroma_minus8);
		...
		MS_DEBUG_DEV_STD(
		  "avg_frame_rate:%d, constant_frame_rate:%d, num_temporal_layers:%d, temporal_id_nested:%d, length_size_minus_one:%d, numOfArrays:%d",
		  dec_conf_rec_p->avg_frame_rate,
		  dec_conf_rec_p->constant_frame_rate,
		  dec_conf_rec_p->num_temporal_layers,
		  dec_conf_rec_p->temporal_id_nested,
		  dec_conf_rec_p->length_size_minus_one,
		  numOfArrays);
		...
		for (int index = 0; index < numOfArrays; index++)
		{
			...
			for (int i = 0; i < hevc_unit.num_nalus; i++)
			{
				...
				{
					return srs_error_new(
					  ERROR_HEVC_DECODE_ERROR, "num_nalus requires 2 only %d bytes", stream->left());
				}
				...
				{
					return srs_error_new(
					  ERROR_HEVC_DECODE_ERROR,
					  "requires %d only %d bytes",
					  data_item.nal_unit_length,
					  stream->left());
				}
				...
				MS_DEBUG_DEV_STD(
				  "hevc nalu type:%d, array_completeness:%d, num_nalus:%d, i:%d, nal_unit_length:%d",
				  hevc_unit.nal_unit_type,
				  hevc_unit.array_completeness,
				  hevc_unit.num_nalus,
				  i,
				  data_item.nal_unit_length);
	...
	srs_error_t SrsFormat::hevc_demux_vps_sps_pps(SrsHevcHvccNalu* nal)
	{
		...
		Utils::RtmpBuffer stream(frame, nb_frame);
	...
	srs_error_t SrsFormat::hevc_demux_vps(Utils::RtmpBuffer* stream)
	{
		...
		{
			return srs_error_new(
			  ERROR_HEVC_DECODE_ERROR, "decode hevc vps requires 1 only %d bytes", stream->left());
		}
		...
		{
			return srs_error_new(
			  ERROR_HEVC_DECODE_ERROR, "hevc forbidden_zero_bit=%d shall be equal to 0", forbidden_zero_bit);
		}

		// nal_unit_type specifies the type of RBSP data structure contained in the NAL unit as
		...
		{
			return srs_error_new(
			  ERROR_HEVC_DECODE_ERROR, "hevc vps nal_unit_type=%d shall be equal to 33", nal_unit_type);
		}
	...
	srs_error_t SrsFormat::hevc_demux_vps_rbsp(char* rbsp, int nb_rbsp)
	{
		...
		// reparse the rbsp.
		Utils::RtmpBuffer stream(rbsp, nb_rbsp);
		...
		if ((err = hevc_demux_rbsp_ptl(&bs, &vps->ptl, 1, vps->vps_max_sub_layers_minus1)) != srs_success)
		{
			return srs_error_wrap(
			  err, "vps rbsp ptl vps_max_sub_layers_minus1=%d", vps->vps_max_sub_layers_minus1);
		}
		...
		{
			return srs_error_new(
			  ERROR_HEVC_DECODE_ERROR, "sublayer flag requires 1 only %d bits", bs.left_bits());
		}
		...
		{
			return srs_error_new(
			  ERROR_HEVC_DECODE_ERROR, "vps maxlayer requires 10 only %d bits", bs.left_bits());
		}
	...
	srs_error_t SrsFormat::hevc_demux_sps(Utils::RtmpBuffer* stream)
	{
		...
		{
			return srs_error_new(
			  ERROR_HEVC_DECODE_ERROR, "decode hevc sps requires 1 only %d bytes", stream->left());
		}
		int8_t nutv = stream->read_1bytes();
		...
		{
			return srs_error_new(
			  ERROR_HEVC_DECODE_ERROR, "hevc sps nal_unit_type=%d shall be equal to 33", nal_unit_type);
		}
	...
	srs_error_t SrsFormat::hevc_demux_sps_rbsp(char* rbsp, int nb_rbsp)
	{
		...
		{
			return srs_error_wrap(
			  err, "sps rbsp ptl sps_max_sub_layers_minus1=%d", sps_max_sub_layers_minus1);
		}
		...
		if (sps->chroma_format_idc == 3)
		{
			if (!bs.require_bits(1))
			{
				return srs_error_new(
				  ERROR_HEVC_DECODE_ERROR,
				  "separate_colour_plane_flag requires 1 only %d bits",
				  bs.left_bits());
			}
		...
		{
			return srs_error_new(
			  ERROR_HEVC_DECODE_ERROR, "conformance_window_flag requires 1 only %d bits", bs.left_bits());
		}
		...
		if (sps->conformance_window_flag)
		{
			...
			vcodec->height -=
			  (sub_height_c * sps->conf_win_bottom_offset + sub_height_c * sps->conf_win_top_offset);
		}
	...
	srs_error_t SrsFormat::hevc_demux_pps(Utils::RtmpBuffer* stream)
	{
		...
		{
			return srs_error_new(
			  ERROR_HEVC_DECODE_ERROR, "decode hevc pps requires 1 only %d bytes", stream->left());
		}
		int8_t nutv = stream->read_1bytes();
		...
		{
			return srs_error_new(
			  ERROR_HEVC_DECODE_ERROR, "hevc pps nal_unit_type=%d shall be equal to 33", nal_unit_type);
		}
	...
	srs_error_t SrsFormat::hevc_demux_pps_rbsp(char* rbsp, int nb_rbsp)
	{
		...
		{
			return srs_error_new(
			  ERROR_HEVC_DECODE_ERROR, "pps slice requires 7 only %d bits", bs.left_bits());
		}
		...
		{
			return srs_error_new(
			  ERROR_HEVC_DECODE_ERROR, "pps slice_chroma_qp requires 6 only %d bits", bs.left_bits());
		}
		...
		if (pps->tiles_enabled_flag)
		{
			...
			{
				return srs_error_new(
				  ERROR_HEVC_DECODE_ERROR, "uniform_spacing_flag requires 1 only %d bits", bs.left_bits());
			}
			...
			if (!bs.require_bits(1))
			{
				return srs_error_new(
				  ERROR_HEVC_DECODE_ERROR,
				  "loop_filter_across_tiles_enabled_flag requires 1 only %d bits",
				  bs.left_bits());
			}
		...
		{
			return srs_error_new(
			  ERROR_HEVC_DECODE_ERROR, "pps loop deblocking filter requires 2 only %d bits", bs.left_bits());
		}
		...
		if (pps->deblocking_filter_control_present_flag)
		{
			if (!bs.require_bits(2))
			{
				return srs_error_new(
				  ERROR_HEVC_DECODE_ERROR,
				  "pps loop deblocking filter flag requires 2 only %d bits",
				  bs.left_bits());
			}
		...
		{
			return srs_error_new(
			  ERROR_HEVC_DECODE_ERROR, "pps scaling_list_data requires 1 only %d bits", bs.left_bits());
		}
		...
		if (!bs.require_bits(1))
		{
			return srs_error_new(
			  ERROR_HEVC_DECODE_ERROR,
			  "lists_modification_present_flag requires 1 only %d bits",
			  bs.left_bits());
		}
		...
		{
			return srs_error_new(
			  ERROR_HEVC_DECODE_ERROR, "extension_present_flag requires 2 only %d bits", bs.left_bits());
		}
		...
		if (pps->pps_extension_present_flag)
		{
			...
			{
				return srs_error_new(
				  ERROR_HEVC_DECODE_ERROR, "pps_range_extension_flag requires 8 only %d bits", bs.left_bits());
			}
		...
		if (pps->pps_range_extension_flag)
		{
			...
			if (!bs.require_bits(2))
			{
				return srs_error_new(
				  ERROR_HEVC_DECODE_ERROR,
				  "cross_component_prediction_enabled_flag requires 2 only %d bits",
				  bs.left_bits());
			}
			...
			if (pps->pps_range_extension.chroma_qp_offset_list_enabled_flag)
			{
				...
				pps->pps_range_extension.cb_qp_offset_list.resize(
				  pps->pps_range_extension.chroma_qp_offset_list_len_minus1);
				pps->pps_range_extension.cr_qp_offset_list.resize(
				  pps->pps_range_extension.chroma_qp_offset_list_len_minus1);
	  ...
	  int max_sub_layers_minus1)
	{
		...
		if (profile_present_flag)
		{
			...
			{
				return srs_error_new(
				  ERROR_HEVC_DECODE_ERROR, "ptl profile requires 88 only %d bits", bs->left_bits());
			}
			  ...
			  ptl->general_profile_idc == 11 || ptl->general_profile_compatibility_flag[11])
			{
				...
				if (
				  ptl->general_profile_idc == 5 || ptl->general_profile_compatibility_flag[5] == 1 ||
			...
			// The number of bits in this syntax structure is not affected by this condition
			if (
			  ptl->general_profile_idc == 1 || ptl->general_profile_compatibility_flag[1] ||
		...
		{
			return srs_error_new(
			  ERROR_HEVC_DECODE_ERROR, "ptl level requires 8 only %d bits", bs->left_bits());
		}
		...
		for (int i = 0; i < max_sub_layers_minus1; i++)
		{
			...
			{
				return srs_error_new(
				  ERROR_HEVC_DECODE_ERROR, "ptl present_flag requires 2 only %d bits", bs->left_bits());
			}
		...
		for (int i = max_sub_layers_minus1; max_sub_layers_minus1 > 0 && i < 8; i++)
		{
			...
			{
				return srs_error_new(
				  ERROR_HEVC_DECODE_ERROR, "ptl reserved_zero requires 2 only %d bits", bs->left_bits());
			}
		...
		for (int i = 0; i < max_sub_layers_minus1; i++)
		{
			if (ptl->sub_layer_profile_present_flag[i])
			{
				if (!bs->require_bits(88))
				{
					return srs_error_new(
					  ERROR_HEVC_DECODE_ERROR,
					  "ptl sub_layer_profile requires 88 only %d bits",
					  bs->left_bits());
				}
				// profile_space  u(2)
				...
				// The number of bits in this syntax structure is not affected by this condition
				if (
				  ptl->sub_layer_profile_idc[i] == 1 || ptl->sub_layer_profile_compatibility_flag[i][1] ||
			...
			if (ptl->sub_layer_level_present_flag[i])
			{
				...
				{
					return srs_error_new(
					  ERROR_HEVC_DECODE_ERROR, "ptl sub_layer_level requires 8 only %d bits", bs->left_bits());
				}
	...
	srs_error_t SrsFormat::avc_demux_sps_pps(Utils::RtmpBuffer* stream)
	{
	...
	srs_error_t SrsFormat::avc_demux_sps()
	{
		...
		Utils::RtmpBuffer stream(sps, nbsps);
		...
		// parameter set or a picture parameter set or a slice of a reference picture or a slice data
	...
	srs_error_t SrsFormat::avc_demux_sps_rbsp(char* rbsp, int nb_rbsp)
	{
		...
		int32_t frame_crop_left_offset = 0, frame_crop_right_offset = 0, frame_crop_top_offset = 0,
		...
		vcodec->width = ((pic_width_in_mbs_minus1 + 1) * 16) - frame_crop_left_offset * 2 -
	...
	srs_error_t SrsFormat::video_nalu_demux(Utils::RtmpBuffer* stream)
	{
		srs_error_t err = srs_success;
		...
		{
			MS_WARN_DEV("avc ignore type=%d for no sequence header", SrsVideoAvcFrameTraitNALU);
			return err;
		}
		...
		else
		{
			...
			{
				vcodec->payload_format =
				  try_annexb_first ? SrsAvcPayloadFormatAnnexb : SrsAvcPayloadFormatIbmf;
			}
			else
			{
				FREEP(err);
				...
				{
					vcodec->payload_format =
					  try_annexb_first ? SrsAvcPayloadFormatIbmf : SrsAvcPayloadFormatAnnexb;
				}
				else
				{
	...
	srs_error_t SrsFormat::avc_demux_annexb_format(Utils::RtmpBuffer* stream)
	{
		srs_error_t err = srs_success;
	...
	srs_error_t SrsFormat::do_avc_demux_annexb_format(Utils::RtmpBuffer* stream)
	{
		srs_error_t err = srs_success;

		// not annexb, try others
		if (!srs_avc_startswith_annexb(stream, nullptr))
		{
	...
	srs_error_t SrsFormat::avc_demux_ibmf_format(Utils::RtmpBuffer* stream)
	{
		srs_error_t err = srs_success;
	...
	srs_error_t SrsFormat::do_avc_demux_ibmf_format(Utils::RtmpBuffer* stream)
	{
		srs_error_t err = srs_success;
		...
		for (int i = 0; i < PictureLength;)
		{
			...
			// TODO: FIXME: Should ignore error? See
			// https://github.com/ossrs/srs-gb28181/commit/a13b9b54938a14796abb9011e7a8ee779439a452
			if (!stream->require(vcodec->NAL_unit_length + 1))
			{
				return srs_error_new(
				  ERROR_HLS_DECODE_ERROR,
				  "PictureLength:%d, i:%d, NaluLength:%d, left:%d",
				  PictureLength,
				  i,
				  vcodec->NAL_unit_length,
				  stream->left());
			}
			...
			if (NALUnitLength < 0)
			{
				return srs_error_new(
				  ERROR_HLS_DECODE_ERROR,
				  "PictureLength:%d, i:%d, NaluLength:%d, left:%d, NALUnitLength:%d",
				  PictureLength,
				  i,
				  vcodec->NAL_unit_length,
				  stream->left(),
	...
	srs_error_t SrsFormat::audio_aac_demux(Utils::RtmpBuffer* stream, int64_t timestamp)
	{
		srs_error_t err = srs_success;
		...
		if (aac_packet_type == SrsAudioAacFrameTraitSequenceHeader)
		{
			...
			{
				char* copy_stream_from = stream->data() + stream->pos();
				acodec->aac_extra_data =
				  std::vector<char>(copy_stream_from, copy_stream_from + aac_extra_size);
		...
		else if (aac_packet_type == SrsAudioAacFrameTraitRawData)
		{
			...
			{
				MS_WARN_DEV("aac ignore type=%d for no sequence header", aac_packet_type);
				return err;
			}
		...
		{
			static int srs_aac_srates[] = { 96000, 88200, 64000, 48000, 44100, 32000, 24000, 22050,
				                              16000, 12000, 11025, 8000,  7350,  0,     0,     0 };
	...
	srs_error_t SrsFormat::audio_mp3_demux(Utils::RtmpBuffer* stream, int64_t timestamp, bool fresh)
	{
		srs_error_t err = srs_success;

		audio->cts = 0;
		audio->dts = timestamp;
		audio->aac_packet_type =
		  fresh ? SrsAudioMp3FrameTraitSequenceHeader : SrsAudioMp3FrameTraitRawData;
		...
		// Update the RAW MP3 data. Note the start is 12 bits syncword 0xFFF, so we should not skip any
		// bytes, for detail please see ISO_IEC_11172-3-MP3-1993.pdf page 20 and 26.
		raw    = stream->data() + stream->pos();
		nb_raw = stream->size() - stream->pos();
	...
	srs_error_t SrsFormat::audio_aac_sequence_header_demux(char* data, int size)
	{
		...
		samplingFrequencyIndex =
		  ((profile_ObjectType << 1) & 0x0e) | ((samplingFrequencyIndex >> 7) & 0x01);
		...
		// @see:
		// https://github.com/winlinvip/nginx-rtmp-module/commit/3a5f9eea78fc8d11e8be922aea9ac349b9dcbfc2
		//
