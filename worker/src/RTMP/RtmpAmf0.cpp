#define MS_CLASS "RTMP::RtmpAmf0"
#define MS_LOG_DEV_LEVEL 3

#include "RTMP/RtmpAmf0.hpp"
#include "RTMP/RtmpJson.hpp"
...
namespace RTMP
{
	...
	void srs_amf0_do_print(RtmpAmf0Any* any, std::stringstream& ss, int level)
	{
		...
		{
			ss << "Date " << std::hex << any->to_date() << "/" << std::hex << any->to_date_time_zone()
			   << std::endl;
		}
		else if (any->is_null())
		{
	...
	srs_error_t srs_amf0_read_string(Utils::RtmpBuffer* stream, std::string& value)
	{
		// marker
		if (!stream->require(1))
		{
	...
	srs_error_t srs_amf0_write_string(Utils::RtmpBuffer* stream, std::string value)
	{
		// marker
		if (!stream->require(1))
		{
	...
	srs_error_t srs_amf0_read_boolean(Utils::RtmpBuffer* stream, bool& value)
	{
		srs_error_t err = srs_success;
	...
	srs_error_t srs_amf0_write_boolean(Utils::RtmpBuffer* stream, bool value)
	{
		srs_error_t err = srs_success;
	...
	srs_error_t srs_amf0_read_number(Utils::RtmpBuffer* stream, double& value)
	{
		srs_error_t err = srs_success;
	...
	srs_error_t srs_amf0_write_number(Utils::RtmpBuffer* stream, double value)
	{
		srs_error_t err = srs_success;
	...
	srs_error_t srs_amf0_read_null(Utils::RtmpBuffer* stream)
	{
		srs_error_t err = srs_success;
	...
	srs_error_t srs_amf0_write_null(Utils::RtmpBuffer* stream)
	{
		srs_error_t err = srs_success;
	...
	srs_error_t srs_amf0_read_undefined(Utils::RtmpBuffer* stream)
	{
		srs_error_t err = srs_success;
	...
	srs_error_t srs_amf0_write_undefined(Utils::RtmpBuffer* stream)
	{
		srs_error_t err = srs_success;
	...
	{
		srs_error_t srs_amf0_read_utf8(Utils::RtmpBuffer* stream, std::string& value)
		{
			srs_error_t err = srs_success;
		...
		srs_error_t srs_amf0_write_utf8(Utils::RtmpBuffer* stream, std::string value)
		{
			srs_error_t err = srs_success;
			...
			if (!stream->require((int)value.length()))
			{
				return srs_error_new(
				  ERROR_RTMP_AMF0_ENCODE, "requires %" PRIu64 " only %d bytes", value.length(), stream->left());
			}
		...
		bool srs_amf0_is_object_eof(Utils::RtmpBuffer* stream)
		{
			// detect the object-eof specially
			if (stream->require(3))
			{
